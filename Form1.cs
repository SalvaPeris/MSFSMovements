    ...
    public partial class Form1 : Form
    {
        ...
        private void button_Connect_Click(object sender, EventArgs e)
        {
            if (my_simconnect == null)
            {
                ...
                {
                    label_status.Text = "Imposible conectar a sim";
                }
            }
            else
            {
        ...
        private void closeConnection()
        {
            if (my_simconnect != null)
            {
                ...
                my_simconnect = null;
                label_status.Text = "Conexión cerrada";
            }
        }
        ...
        private void initDataRequest()
        {
            try
            {
                ...
                my_simconnect.AddToDataDefinition(DEFINITIONS.Struct1, "Ground Altitude", "meters", SIMCONNECT_DATATYPE.FLOAT64, 0.0f, SimConnect.SIMCONNECT_UNUSED); my_simconnect.AddToDataDefinition(DEFINITIONS.Struct1, "Plane Roll", "radians", SIMCONNECT_DATATYPE.FLOAT64, 0.0f, SimConnect.SIMCONNECT_UNUSED);
                my_simconnect.AddToDataDefinition(DEFINITIONS.Struct1, "PLANE PITCH DEGREES", "degrees", SIMCONNECT_DATATYPE.FLOAT64, 0.0f, SimConnect.SIMCONNECT_UNUSED);
                my_simconnect.AddToDataDefinition(DEFINITIONS.Struct1, "Plane Bank Degrees", "degrees", SIMCONNECT_DATATYPE.FLOAT64, 0.0f, SimConnect.SIMCONNECT_UNUSED);
        ...
        {
            label_status.Text = "Conectado al SIMULADOR y al COM";
        ...
        {
            label_status.Text = "sim ha salido";
            closeConnection();
            timer1.Enabled = false;
        }
        ...
        private void simconnect_OnRecvSimobjectDataBytype(SimConnect sender, SIMCONNECT_RECV_SIMOBJECT_DATA_BYTYPE data)
        {
            ...
            if (data.dwRequestID == 0)
            {
                ...
                textBox_pitch.Text = (struct1.pitch * radiansToDegrees).ToString();
                textBox_roll.Text = (struct1.roll * radiansToDegrees).ToString().ToString();
                ...
                send_Serial(struct1.roll * radiansToDegrees);
        ...
        private void send_Serial(double rollDegrees)
        {
            var rollToServo = MapRollToServoAngle(rollDegrees);
            textbox_roll_s.Text = rollToServo.ToString();

            if (_serialPort.IsOpen)
            {
                _serialPort.WriteLine(rollDegrees.ToString());
            }
            else
            {
                label_status.Text = "El puerto serial no está abierto.";
            }
        }

        int MapRollToServoAngle(double roll)
        {
            double rollMin = -180.0;
            double rollMax = 180.0;
            int servoMin = 0;
            int servoMax = 180;

            roll = Math.Max(rollMin, Math.Min(rollMax, roll));

            int servoAngle = (int)((roll - rollMin) * (servoMax - servoMin) / (rollMax - rollMin) + servoMin);
            return servoAngle;
        }
        ...
        private void timer1_Tick_1(object sender, EventArgs e)
        {
            ...
            label_status.Text = "Enviando petición...";
        }
